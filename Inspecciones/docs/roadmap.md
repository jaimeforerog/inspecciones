...
**Estado general:** Diseño 98% · Fase 3 en curso (slices 1a-1o + erp-1..erp-4 cerrados) · 2 pasos bloqueados por externos (0.13 ADR-002 + 0.18 DDL preop) · 1 bloqueo cross-team adicional (M-1 OT en Maquinaria_V4 — slice 8 pausado por DDL DBA)
...
|---|---|
| ✅ | Completado |
| 🟡 | Parcialmente completado — algunos sub-items cerrados, otros pendientes |
| 🔄 | En curso |
...
| # | Paso | Estado | Notas |
|---|---|---|---|
| 3.1 | Setup solución .NET con estructura modular | ✅ | Bootstrap commit `4c9812a` (2026-05-04). `Inspecciones.Domain`, `.Application`, `.Infrastructure`, `.Api` + tests. |
| 3.2 | Configurar Marten event store | ✅ | Configurado en `Program.cs`. Identidad explícita para catálogos ERP (`EquipoLocal`, `RutinaTecnicaLocal`, `RepuestoLocal`) por convención `<X>Id` (no `Id`). Outbox Wolverine integrado (ADR-006). |
| 3.3 | Configurar Wolverine (mediator + outbox) | ✅ | `Program.cs` con `UseWolverine` + `IntegrateWithWolverine` + `UseDurableLocalQueues` + auto-discovery de handlers. |
| 3.4 | Instrumentación con OpenTelemetry / App Insights | ⏳ | Tracing distribuido — pendiente. |
| 3.5 | Repositorio + CI/CD inicial | ✅ | GitHub Actions activo desde bootstrap. |

### 3.B Aggregate `InspeccionTecnica`

> **Estado a 2026-05-07:** primer corte del aggregate cerrado vía slices 1a-1f (TDD). Cubre lifecycle inicial, hallazgos completos, repuestos parciales. Pendientes: firma/dictamen, adjuntos, sagas OT.

| # | Paso | Estado | Notas |
|---|---|---|---|
| 3.6 | Implementar value objects (`UbicacionGps`, `Hallazgo`, etc.) | ✅ | `UbicacionGps`, `LecturaMedidor`, `Hallazgo`, `Repuesto`, value objects de Inspeccion implementados (slices 1a-1f). |
| 3.7 | Eventos lifecycle (5): `InspeccionIniciada`, `Firmada`, `Cerrada`, `CerradaSinOT`, `Cancelada` | 🟡 | `InspeccionIniciada_v1` cerrado en slice 1a. Faltan `Firmada`, `Cerrada`, `CerradaSinOT`, `Cancelada`. |
| 3.8 | Eventos hallazgos (3): `HallazgoRegistrado`, `Actualizado`, `Eliminado` | ✅ | Cerrados en slices 1c, 1d, 1e. Soft delete con `HallazgoEliminado_v1`. `HallazgoRegistrado_v1` con `Origen ∈ {PreOperacional, Manual, Seguimiento}` + `NovedadPreopOrigenId?` + `SeguimientoOrigenId?` (trazabilidad cross-inspección). Invariantes I-H10/I-H11. |
| 3.9 | Evento `NovedadPreopDescartada` | ⏳ | NO crea hallazgo. Pendiente. |
| 3.10 | Eventos repuestos (3): `RepuestoEstimado`, `Actualizado`, `Removido` | 🟡 | `RepuestoEstimado_v1` cerrado en slice 1f. Faltan `Actualizado`, `Removido`. |
| 3.11 | Eventos adjuntos (2): `AdjuntoSubido`, `Eliminado` | ⏳ | Pattern SAS upload. Pendiente. |
| 3.12 | Eventos firma atómicos (3): `Diagnostico`, `Dictamen`, `Firmada` | ⏳ | Comando consolidado `FirmarInspeccion`. Pendiente. |
| 3.13 | Evento `OTGeneracionFallida` | ⏳ | Estado `CierrePendienteOT`. Pendiente. |
| 3.14 | Comandos + handlers (15) | 🟡 | 5/15 cerrados: `IniciarInspeccion` (1b), `RegistrarHallazgo` (1c), `ActualizarHallazgo` (1d), `EliminarHallazgo` (1e), `AsignarRepuesto` (1f). |
| 3.15 | Validaciones pre-firma V-F1 a V-F7 | ⏳ | En el handler `FirmarInspeccion`. Pendiente. |
| 3.16 | Tests unitarios del aggregate | ✅ | Cobertura ≥85% por slice (regla CLAUDE.md). Rebuild tests obligatorios. xUnit + FluentAssertions + Marten embebido. |
...
| 3.27c | Adapter MYE: PUT /equipos/{id}/dictamen-vigente (NUEVO 2026-04-30) | ✅ | **Cerrado slice-erp-3 (commit `28de25b`, 2026-05-19).** Listener `SincronizarDictamenVigenteListener` reactivo a `InspeccionFirmada_v1` propaga via `PUT /equipos/{id}/dictamen-vigente` (M-W-1). Mapeo enum→int (`PuedeOperar→0`, `ConRestriccion→1`, `NoPuedeOperar→2`). Puerto `IInspeccionReader` + `MartenInspeccionReader`. 11 tests verde con `FakeInspeccionReader`. Política ADR-006 aplicada (5xx retry, 4xx + `ArgumentException` dead-letter). Pendiente: tests de integración Marten real (FU-45) + propagación de JWT del request entrante (FU-14 / FU-44 abierto). |
...
| 3.24c | Saga `SincronizarDictamenVigenteSaga` (NUEVA 2026-04-30) | ✅ | **Cerrada slice-erp-3 (commit `28de25b`, 2026-05-19).** Implementada como listener Wolverine `SincronizarDictamenVigenteListener` (no saga "stateful" — pattern reactivo simple). Reacciona a `InspeccionFirmada_v1`, reconstruye aggregate via `IInspeccionReader.AggregateStreamAsync<Inspeccion>` para recuperar `Dictamen` + `EquipoId` (no viajan en el evento), invoca paso 3.27c. Independiente del flujo OT. |
...
| 3.29 | Adapter Preop: POST /preop/novedades/descartar (1 novedad por llamada en MVP) | ✅ | **Cerrado slice-erp-2 (commit `63082fa`, 2026-05-19).** Listener Wolverine `DescartarNovedadPreopErpListener` reactivo a `NovedadPreopDescartada_v1` propaga cierre via `CerrarPreoperacionalFallasAsync` (P-6 en Maquinaria_V4, ruta real `POST /api/preoperacional-fallas/cerrar`). Idempotencia natural por `PodId` (`200 yaCerradas` / `409 YA_CERRADO` = éxito silencioso). Política ADR-006 aplicada (5xx con retry 5s→30s→2m→10m, 4xx + `ArgumentException` → dead-letter inmediato). 11 tests verde. |
...
| # | Paso | Estado | Notas |
|---|---|---|---|
| 3.32 | Sync inicial al primer login + sync delta on-app-open (sin cron nocturno) | 🟡 | **Parcialmente cerrado slice-erp-4 (commit `fb44741`, 2026-05-19).** Endpoint `POST /api/v1/catalogos/sync` operativo con wipe-and-replace + ETag por catálogo (`CatalogoSyncState` document Marten). Cubre **3 catálogos globales puros: causas-falla, tipos-falla, productos**. `If-None-Match` → `304 Not Modified` deja cache local intacto. Body vacío → `"vaciado-sospechoso"` con cache intacto. Partial-failure por catálogo aislada via `LightweightSession` propia. 23 tests verde. Pendiente: agregar equipos + partes (per-equipo, no globales — requiere otro pattern) + rutinas-monitoreo per-equipo (M-16 — divergencia 2026-05-13 vs cliente-side por grupo, ver §0.4 contrato ERP) + capability gate (FU-52) + test E2E con Postgres real (FU-51). ADR-004 canonical: bootstrap PWA dispara sync con `If-None-Match`. |
...
Fase 3 (Backend):             ████████████░░░░░░░░  ~60% 🟡 (slices 1a-1o + erp-1..erp-4 cerrados — 15/15 comandos del aggregate + outbox/saga ERP + sync catálogos parcial)
Fase 4 (APIs Sinco):          █████░░░░░░░░░░░░░░░  ~30% 🟡 (reconciliación bilateral 2026-05-13 con Maquinaria_V4 cerrada — 9 endpoints acoplables operativos; M-1 sigue bloqueante por DDL DBA)
Fase 5 (Frontend):            ░░░░░░░░░░░░░░░░░░░░   0% ⏳
...
**Slices cerrados (Fase 3.B aggregate `Inspeccion` + 3.B' monitoreo + 3.D sagas/integración):**
- ✅ slice-1a — `IniciarInspeccion` aggregate puro (commit `fa1323a`)
- ✅ slice-1b — `IniciarInspeccion` handler + `InspeccionAbiertaPorEquipoView` (commit `045b359`)
- ✅ slice-1c — `RegistrarHallazgo` con `Origen ∈ {Manual, PreOperacional}` (commit `6cf6601`)
- ✅ slice-1d — `ActualizarHallazgo` (commits `141e3f4`, `8394143`)
- ✅ slice-1e — `EliminarHallazgo` soft delete (commit `1a21830`)
- ✅ slice-1f — `AsignarRepuesto` (commit `5ed029c`)
- ✅ slice-1g — `FirmarInspeccion` (commit `b2e355d`)
- ✅ slice-1h — `IniciarInspeccionMonitoreo` (commit `8888d2e`)
- ✅ slice-1i — `RegistrarMedicion` (commit `b93834c`)
- ✅ slice-1i' — `RegistrarEvaluacionCualitativa` (commit `c957e54`)
- ✅ slice-1j — `OmitirItemMonitoreo` (commit `f0cb808`)
- ✅ slice-1k — `GenerarOT` (commit `475cfeb`)
- ✅ slice-1l — `RechazarGenerarOT` (commit `acc61b1`)
- ✅ slice-1m — `CancelarInspeccion` (commit `fb63ab7`)
- ✅ slice-1n — `DescartarNovedadPreop` (commit `fb2c033`)
- ✅ slice-1o — `ActualizarRepuesto` (commit `36062e0`)

**Slices cerrados (Acople ERP Maquinaria_V4 — sub-track 2026-05-19):**
- ✅ slice-erp-1 — adapter `IMaquinariaErpClient` + 11 endpoints admin + tests WireMock (commit `4c2ef4e`)
- ✅ slice-erp-2 — `DescartarNovedadPreop` outbox listener → P-6 (commit `63082fa`)
- ✅ slice-erp-3 — `SincronizarDictamenVigente` saga → M-W-1 (commit `28de25b`)
- ✅ slice-erp-4 — sync on-app-open de 3 catálogos vía `POST /api/v1/catalogos/sync` con ETag (commit `fb44741`)
- ✅ docs reconciliación bilateral 2026-05-13 en §0 de `06-contrato-apis-erp.md` (commit `8c3cb62`)

**Próximos 3 hitos críticos:**
1. **Continuar Fase 3** — adjuntos (3.11 pattern SAS upload), aggregate `SeguimientoHallazgo` (3.C — `SeguimientoAbierto`/`Resuelto`/`Escalado`), sagas remanentes (`CerrarInspeccionSaga` 3.24, `EjecutarOTSaga` 3.24b, `GenerarPdfInspeccionSaga` 3.24d).
2. **Fase 1 — Arrancar fundaciones Azure en paralelo** (bloqueante de Fase 2 y del despliegue real de Fase 3).
3. **Desbloqueos cross-team:** (a) M-1 `POST /mye/ot-correctivas` — slice 8 Maquinaria_V4 pausado por DDL DBA; (b) ADR-002 cierre del mecanismo de identidad del host PWA (FU-14 + FU-44 + FU-52); (c) FU-50 confirmar con David la matriz parte↔producto para BOM real en repuestos.
