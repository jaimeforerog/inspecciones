...
- **Fase actual:** Fase 1 ampliamente avanzada. **15 slices cerrados** del aggregate `Inspeccion` (1a..1o) + 4 fixes de followups (FU-32/36/37/38), más **sub-track de acople ERP a Maquinaria_V4 cerrado** (slices `erp-1..erp-4`), más **sub-track multi-tenancy CERRADO** (slices mt-1..mt-4: JWT claims pipeline + Marten Conjoined + propagación JWT al ERP + E2E isolation/observabilidad/cierre, 2026-05-19). HEAD del sub-track aggregate: `36062e0 feat(slice-1o): ActualizarRepuesto` (2026-05-11). HEAD del sub-track ERP + multi-tenancy: ver §§"Acople ERP Maquinaria_V4" y "Multi-tenancy (sub-track 2026-05-19)" abajo. **Sub-track multi-tenancy listo para piloto multi-empresa** sujeto a checklist en `slices/mt-4-e2e-isolation-observability/baseline-piloto.md`.
...
- **slice-mt-4** (cerrado 2026-05-19, commit `feat(slice-mt-4): E2E cross-tenant isolation + observabilidad por IdEmpresa + cierre sub-track`): pieza final del sub-track multi-tenancy. Nuevos en `src/Inspecciones.Infrastructure/Auth/`: `IncomingBearerCarrier` (AsyncLocal estático del bearer del request HTTP entrante), `CaptureBearerForOutboxMiddleware` (ASP.NET middleware), `ForwardAuthEnvelopeRule` (`IEnvelopeRule` que escribe `X-Forwarded-Authorization` en cada envelope outgoing del outbox — cierra FU-60 que mt-3 dejó pendiente), `SessionLoggingScope.BeginEmpresaScope` (extension con `IdEmpresa`/`IdUsuario` + `Activity.AddTag("id_empresa")`), `SessionLoggingScopeFilter` (`IEndpointFilter` global que aplica el scope a los 15 endpoints sin tocarlos uno por uno — D-MT4-2'). Nuevo en `src/Inspecciones.Infrastructure/Erp/`: `InspeccionesMeters` con `Meter("Inspecciones")` BCL + counter `inspecciones.erp.calls` taggeado por `id_empresa`/`endpoint`/`resultado`. Ambos listeners ERP registran la métrica en éxito/fallo/idempotencia. `SincronizarDictamenVigenteListener.LogSyncFallida` gana `TenantId` (simetría con `DescartarNovedadPreopErpListener`). Cableado `Program.cs`: middleware después del corporativo, `opts.Policies.AllSenders(cfg => cfg.CustomizeOutgoing(rule.Modify))` (decisión green D-MT4-1': `AddOutgoingRule` no expuesto en interfaz pública), `MapGroup` con filter envuelve los endpoints. Tests nuevos: `Infrastructure.Tests` +10 (`CaptureBearerForOutboxMiddlewareTests` 7 + `SessionLoggingScopeTests` 2 + extras); `Domain.Tests` +2 (`RebuildCrossTenantDefensivoTests` cierra FU-59 — verifica MT4-INV-4 sin Marten); `Api.Tests` +4 (`CrossTenantE2EIsolationTests` 2 con paralelismo de 20 tareas + `CaptureBearerForOutboxEndToEndTests` 2 — requieren Postgres). Cierra FU-56, FU-59, FU-60. Abre FU-63 (skip determinístico Api.Tests) y FU-64 (test del counter de métricas). Documento `baseline-piloto.md` con checklist operativo para primer deploy multi-empresa. **Sub-track multi-tenancy cerrado.**

### 🟡 Salud del repo (revisión 2026-05-19, vigente al cierre de mt-4)

Estado factual al cierre del slice mt-4 (Domain.Tests + Infrastructure.Tests verificados local sin Docker en esta sesión; Api.Tests/Application.Tests requieren Postgres/Docker no disponible en esta sesión — FU-47/FU-63 abiertos):

- **Build:** limpio en los 8 proyectos. `TreatWarningsAsErrors=true` vigente. 0 warnings, 0 errors.
- **Tests dominio:** `Domain.Tests` 248/267 pass + 19 skip esperados (+2 del slice mt-4 `RebuildCrossTenantDefensivoTests`). Cobertura ramas aggregate `Inspeccion`: 94.44% sin cambios (D3 vigente desde mt-1 — mt-4 NO toca el aggregate).
- **Tests API HTTP:** `Api.Tests` 73 + 4 nuevos del slice mt-4 (`CrossTenantE2EIsolationTests` 2 + `CaptureBearerForOutboxEndToEndTests` 2) + 7 skip históricos = **77/84 pass + 7 skip cuando Postgres disponible**. Sin Postgres ni Docker, `InspeccionesAppFactory` falla en construcción (limitación heredada de mt-2, no regresión de mt-4). FU-63 abierto para skip determinístico.
- **Tests Application:** `Application.Tests` requiere Docker (Testcontainers). `FU-47` preexistente — no es regresión de mt-4.
- **Tests Infrastructure:** `Infrastructure.Tests` **103/103 verde** (slices erp-1..erp-4 + mt-2 + mt-3 + mt-4). mt-4 añadió 10 tests: 7 `CaptureBearerForOutboxMiddlewareTests` (6 del spec §6.1..§6.6 + 1 nesting) + 2 `SessionLoggingScopeTests` (BeginEmpresaScope happy + ClaimRequerida defensivo) + 1 extra IncomingBearerCarrier. Patrón "puerto + fake" preservado.
...
- **Observabilidad por `IdEmpresa` (regla nueva mt-4 — MT4-INV-3):** los logs estructurados de handlers/listeners críticos deben incluir `IdEmpresa` (o `TenantId`) como propiedad estructurada para permitir filtrado en App Insights / Azure Monitor. Para endpoints HTTP: el `SessionLoggingScopeFilter` ya lo aplica globalmente — los nuevos endpoints lo heredan sin código. Para listeners Wolverine ERP: el patrón estándar es `[LoggerMessage]` template con `TenantId={TenantId}` enriquecido desde `envelope.TenantId` (referencia: `DescartarNovedadPreopErpListener.LogCierreFallido`, `SincronizarDictamenVigenteListener.LogSyncFallida`). **Prohibido loguear el JWT raw** — solo `IdEmpresa`/`IdUsuario` (enteros, no PII). El bearer entrante se persiste solo en `wolverine_outgoing_envelopes.headers` (Postgres del módulo, red privada — aceptado por D-MT3-2). Métricas via `Meter("Inspecciones")` BCL (`InspeccionesMeters`) con tag `id_empresa` cuando aplique.
